...
@Component({
  ...
  template: `
    <div class="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div class="text-center mb-8">
        ...
        <p class="text-gray-600">Tu pedido #{{ orderTrackingCode }} está en proceso</p>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-gradient-to-r from-[#a81b8d] to-[#8b1574] rounded-lg p-8 text-white relative overflow-hidden">
            <!-- Decoración de fondo -->
          ...
          <div class="bg-white rounded-lg border border-gray-200 p-6">
            <h2 class="text-xl font-semibold text-gray-900 mb-6">Estado del pedido</h2>
                  ...
                  <div class="flex flex-col items-center mr-4">
                    <div 
                      ...
                      [ngClass]="{
                        ...
                        'bg-gray-200 text-gray-400': !step.completed && !step.current
                      }"
                    >
                      @if (step.completed) {
                  ...
                  <div class="flex-1 pt-1">
                    ...
                    <p class="text-sm text-gray-500">
                      @if (step.completed) {
                        ...
                        <span class="text-[#a81b8d] animate-pulse">En proceso...</span>
                      } @else {
                        <span>Pendiente</span>
                      }
                    </p>
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-6">
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                  ...
                  <div>
                    <h3 class="font-semibold text-gray-900">{{ deliveryPerson.firstName }} {{ deliveryPerson.lastName }}</h3>
                    <p class="text-sm text-gray-600">{{ deliveryPerson.vehicleType }}</p>
                  </div>
                </div>
                <a 
          ...
          <div class="bg-white rounded-lg border border-gray-200 p-6">
                ...
                @for (item of orderItems; track item.id) {
                  <div class="flex items-center justify-between pb-3 border-b border-gray-100 last:border-b-0">
                    <div class="flex items-center space-x-3">
                      @if (item.image || item.product?.thumbnail || item.product?.resources?.[0]?.url) {
                        <img 
                        ...
                        />
                        <div class="w-12 h-12 bg-gray-200 rounded items-center justify-center text-lg hidden">📦</div>
                      } @else {
                        <div class="w-12 h-12 bg-gray-200 rounded flex items-center justify-center text-lg">📦</div>
                      }
                      <div>
                    ...
                    <p class="font-semibold text-gray-900">S/ {{ getItemPrice(item) }}</p>
                  </div>
                }
              }
            </div>
            ...
            <div class="mt-4 pt-4 border-t border-gray-200 space-y-2">
              ...
              <div class="flex justify-between text-lg font-bold text-gray-900 pt-2 border-t border-gray-200">
                ...
                <span class="text-[#a81b8d]">S/ {{ totalAmount.toFixed(2) }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="lg:col-span-1 space-y-6">
          ...
          <div class="bg-white rounded-lg border border-gray-200 p-6">
            <h3 class="font-semibold text-gray-900 mb-4 flex items-center">
              <svg class="w-5 h-5 mr-2 text-[#a81b8d]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="text-sm text-gray-600">
              @if (deliveryAddress.name) {
                <p class="font-semibold text-gray-900 mb-2">{{ deliveryAddress.name }}</p>
              }
              <p class="text-gray-700 leading-relaxed">{{ deliveryAddress.fullAddress || deliveryAddress.street }}</p>
              @if (deliveryAddress.references) {
                <p class="mt-2 text-gray-500 italic">Ref: {{ deliveryAddress.references }}</p>
              }
            </div>
          </div>
          ...
          <div class="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div class="flex items-center space-x-3">
              <img [src]="paymentMethodIcon" [alt]="paymentMethodName" class="h-8 w-auto" />
              <div>
          ...
          <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg border border-gray-200 p-6">
            <h3 class="font-semibold text-gray-900 mb-3">Código de seguimiento</h3>
            <div class="bg-white rounded-lg p-3 text-center border-2 border-dashed border-gray-300">
              <p class="text-2xl font-mono font-bold text-[#a81b8d]">{{ orderTrackingCode }}</p>
            </div>
            <p class="text-xs text-gray-500 mt-2 text-center">Usa este código para consultar tu pedido</p>
          </div>
          ...
          <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <div class="flex items-start space-x-2">
              ...
              <div class="text-sm text-yellow-800">
                ...
                <p>Contáctanos al <a href="tel:016511111" class="underline font-medium">01-651-1111</a></p>
              </div>
            </div>
          </div>
        </div>
  ...
  styles: [`
    ...
    .motorcycle-icon {
      animation: motorcycle-move 2s ease-in-out infinite, motorcycle-bounce 1s ease-in-out infinite;
...
export class OrderTrackingComponent implements OnInit, OnDestroy {
  ...
  deliveryCost: number = 5.00;  // Costo de envío estándar
  ...
  loadOrderDetails(): void {
    this.orderService.getOrderById(this.orderId).subscribe({
      next: (order: Order) => {
        this.orderTrackingCode = order.delivery?.trackingCode || this.generateTrackingCode();
        this.currentStatus = order.orderStatus;
        this.totalAmount = order.totalAmount;
        this.estimatedMinutes = order.delivery?.estimatedTime || 35;
        
        // Mapear items
        this.orderItems = (order.orderItemList || order.orderItems || []).map(item => {
          // Obtener la imagen principal del producto
          const primaryResource = item.product.resources?.find(r => r.isPrimary);
          const imageUrl = primaryResource?.url || item.product.thumbnail || 'assets/products/default.webp';
    ...
    if (trackingData) {
      try {
        ...
        if (tracking.deliveryLocation) {
          const location = typeof tracking.deliveryLocation === 'string' 
          ...
          if (location.name || (location.address && location.district && !location.lat)) {
            ...
            console.log('📍 Dirección cargada desde deliveryAddress (tracking):', this.deliveryAddress);
          } else {
            // Formato antiguo del mapa (tiene lat/lng)
            const fullAddress = location.address || 'Av. Javier Prado 1234, San Isidro';
            ...
            console.log('📍 Dirección cargada desde mapa (tracking):', this.deliveryAddress);
          }
        } else {
  ...
  startStatusSimulation(): void {
    ...
    const statuses = [OrderStatus.PREPARING, OrderStatus.OUT_FOR_DELIVERY, OrderStatus.DELIVERED];
    
    this.statusUpdateInterval = setInterval(() => {
      if (simulationStep < statuses.length && this.currentStatus !== OrderStatus.DELIVERED) {
        ...
        if (this.currentStatus === OrderStatus.OUT_FOR_DELIVERY && !this.deliveryPerson) {
          this.deliveryPerson = {
            firstName: 'Carlos',
            lastName: 'Rodríguez',
  ...
  private loadDeliveryAddressFromLocalStorage(): void {
    ...
    if (addressData) {
      try {
        ...
        console.log('📍 Dirección cargada desde deliveryAddress:', this.deliveryAddress);
        return;
      } catch (e) {
    ...
    if (locationData) {
      try {
        ...
        console.log('📍 Dirección cargada desde deliveryLocation (fallback):', this.deliveryAddress);
      } catch (e) {
    ...
    this.orderItems.forEach(item => {
      ...
      itemsHTML += `
        <tr>
          ...
          <td style="padding: 8px; text-align: right; border-bottom: 1px solid #e5e7eb;">S/ ${(itemTotal / item.quantity).toFixed(2)}</td>
          <td style="padding: 8px; text-align: right; border-bottom: 1px solid #e5e7eb;">S/ ${itemTotal.toFixed(2)}</td>
        </tr>
      `;
    });
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <div class="info-section">
          ...
          ${this.deliveryAddress.name ? `<div style="margin: 5px 0;"><strong>${this.deliveryAddress.name}</strong></div>` : ''}
          <div style="margin: 5px 0;">${this.deliveryAddress.fullAddress || this.deliveryAddress.street}</div>
          ${this.deliveryAddress.references ? `<div style="margin: 5px 0; color: #6b7280; font-style: italic;">Ref: ${this.deliveryAddress.references}</div>` : ''}
        </div>

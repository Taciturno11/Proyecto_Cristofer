                ...
                <div class="flex items-start space-x-4">
                  ...
                  <div class="flex-shrink-0">
                    @if (item.product.thumbnail || item.product.resources?.[0]?.url) {
                      ...
                      />
                      <div class="h-20 w-20 bg-gray-200 rounded-md border items-center justify-center text-2xl hidden">📦</div>
                    } @else {
                      <div class="h-20 w-20 bg-gray-200 rounded-md border flex items-center justify-center text-2xl">📦</div>
                    }
                  </div>
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div>
                      @if (item.product.discountedPrice && item.product.discountedPrice < item.product.price) {
                        <span class="text-sm text-gray-500 line-through mr-2">S/ {{ item.product.price.toFixed(2) }}</span>
                        <span class="text-lg font-semibold text-[#a81b8d]">S/ {{ item.product.discountedPrice.toFixed(2) }}</span>
                      } @else {
                        <span class="text-lg font-semibold text-[#a81b8d]">S/ {{ item.product.price.toFixed(2) }}</span>
                      }
                    </div>
                  </div>
            ...
            <div class="bg-gray-50 rounded-lg p-6 sticky top-32">
                ...
                @for (item of cart.items; track item.product.id) {
                  <div class="flex justify-between text-sm py-2 border-b border-gray-200">
                    ...
                    <div class="text-right">
                      @if (item.product.discountedPrice && item.product.discountedPrice < item.product.price) {
                        <p class="text-gray-900 font-medium">S/ {{ item.subtotal.toFixed(2) }}</p>
                        <p class="text-xs text-gray-500 line-through">S/ {{ (item.product.price * item.quantity).toFixed(2) }}</p>
                      } @else {
                        <p class="text-gray-900 font-medium">S/ {{ item.subtotal.toFixed(2) }}</p>
                      }
                    </div>
              ...
              <div class="space-y-3 mb-4 pt-3 border-t-2 border-gray-300">
                <div class="flex justify-between text-sm font-medium">
                  <span class="text-gray-900">Subtotal ({{ getTotalItems() }} productos)</span>
...
export class ShoppingCartComponent implements OnInit, OnDestroy {
  ...
  /**
   * Obtiene el precio efectivo del producto (con descuento si está disponible)
   */
  ...
  proceedToCheckout(): void {
    ...
    // Si está autenticado, navegar a la página de ubicación de tienda

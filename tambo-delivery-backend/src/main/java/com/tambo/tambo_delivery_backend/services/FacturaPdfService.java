...
public class FacturaPdfService {
    ...
    public byte[] generateFacturaPdf(Order order) {
        try {
            ...
            // ======= CABECERA =======
            Paragraph title = new Paragraph("🧾 Factura - Tambo Delivery", titleFont);
    ...
    public void enviarFacturaPorCorreo(Order order) {
        try {
            byte[] pdfBytes = generateFacturaPdf(order);

            MimeMessage message = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true);

            helper.setTo(order.getUser().getEmail());
            helper.setSubject("Tu factura de compra - Tambo");
            helper.setText("Gracias por tu compra. Adjuntamos la factura en PDF.");

            helper.addAttachment("factura_tambo.pdf", new ByteArrayResource(pdfBytes));

            mailSender.send(message);
            System.out.println("Factura enviada al correo: " + order.getUser().getEmail());
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("Error al enviar factura por correo: " + e.getMessage());
        }
    }
